...
export function MessageInput({ roomId }: Props) {
  ...
  const sendMessage = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!profileId) throw new Error('ログイン情報が見つかりません。');
      ...
      window.alert(e instanceof Error ? e.message : 'メッセージ送信に失敗しました。');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <form
      ...
      className="fixed inset-x-0 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-20 border-t border-white/10 bg-panel/95 px-3 py-2 backdrop-blur"
    >
